...
public class StaxParser {
	...
	private static final String EMAILOFRECEIVER = "emailOfReceiver";
	private static final String SUBJECT = "subject";
	private static final String BODYOFEMAIL = "bodyOfEmail";
    ...
    private void proceedStartElement(XMLEvent xmlEvent, XMLEventReader xmlEventReader) throws XMLStreamException
    {
        if (xmlEvent.isStartElement())
        {
            ...
            else if (isTagNameEqual(startElement, EMAILOFRECEIVER))
            {
            	email.setEmailOfReceiver(xmlEventReader.nextEvent().asCharacters().getData());
            }
            else if (isTagNameEqual(startElement, SUBJECT))
            {
            	email.setSubject(xmlEventReader.nextEvent().asCharacters().getData());
            }
            else if (isTagNameEqual(startElement, BODYOFEMAIL))
            {
            	email.setBodyOfEmail(xmlEventReader.nextEvent().asCharacters().getData());
            }
        }
    }
